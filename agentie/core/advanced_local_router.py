...
def try_advanced_local_command(message: str) -> dict | None:
    ...
    if date_diff:
        start=datetime.fromisoformat(date_diff.group(1)); end=datetime.fromisoformat(date_diff.group(2)); sec=(end-start).total_seconds()
        return {"message":f"The difference is {sec/86400:.4g} days.","card":{"type":"date_difference","start":start.isoformat(),"end":end.isoformat(),"seconds":sec,"days":sec/86400}}

    json_match = re.match(r"^format json:\s*(.+)$", text, re.IGNORECASE)
    if json_match:
    ...
    yaml_match = re.match(r"^format yaml:\s*(.+)$", text, re.IGNORECASE)
    if yaml_match:
    ...
    scratch_get = re.match(r"^(?:scratchpad get|read scratchpad)\s+(.+)$", text, re.IGNORECASE)
    if scratch_get:
        data=_load(advanced.SCRATCHPAD,{}); key=scratch_get.group(1).strip(); value=data.get(key)
    ...
    unzip_match=re.match(r"^(?:unzip|extract)\s+([\w.-]+\.zip)$",text,re.IGNORECASE)
    if unzip_match:
