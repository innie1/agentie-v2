...
def _slug(value: str) -> str:
    return re.sub(r"[^a-z0-9]+", "-", value.lower()).strip("-")[:60] or "agent"
...
def create_agent(
    ...
    existing = next((item for item in agents if str(item.get("name", "")).casefold() == name.casefold()), None)
    if existing:
...
def update_agent_manager(agent_id_or_name: str, manager_id_or_name: str | None) -> dict[str, Any]:
    data = _load()
    agents = data.setdefault("agents", [])
    target_key = _clean(agent_id_or_name).casefold()
    target = next((x for x in agents if str(x.get("id", "")).casefold() == target_key or str(x.get("name", "")).casefold() == target_key), None)
    if not target:
