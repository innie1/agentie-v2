...
def _public(agent: dict[str, Any]) -> dict[str, Any]:
    return {
        "id": agent.get("id"), "name": agent.get("name"), "role": agent.get("role"), "base": agent.get("base"),
        "purpose": agent.get("purpose", ""), "manager_id": agent.get("manager_id"), "status": agent.get("status", "idle"),
        "memory_scope": agent.get("memory_scope"), "session_prefix": agent.get("session_prefix"),
        "skills": list(agent.get("skills") or []), "permissions": dict(agent.get("permissions") or {}),
        "created_at": agent.get("created_at"), "updated_at": agent.get("updated_at"),
    }


def list_agents() -> list[dict[str, Any]]:
    return [_public(item) for item in _load().get("agents", [])]
