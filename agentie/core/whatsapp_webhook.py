...
# Compatibility helper used by regression coverage. This is deliberately not a
# route; main.py is the only /platform.js owner.
...
async def automation_external_webhook(event_type: str, request: Request):
    if not webhook_allowed(
        ...
        request.headers.get("x-agentie-webhook-token"),
    ):
...
# Existing richer-thread compatibility endpoints. The connected group-chat
# surface itself lives in platform_next4_api and is included directly by main.
