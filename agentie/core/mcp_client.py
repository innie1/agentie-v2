...
async def inspect_server(name: str) -> dict[str, Any]:
    ...
    async with _client_for(server) as client:
        ...
        templates = [{"name": getattr(i, "name", None), "title": getattr(i, "title", None), "uri_template": str(getattr(i, "uri_template", "")), "description": getattr(i, "description", None)} for i in (getattr(template_result, "resource_templates", []) or [])]
        info = getattr(client, "server_info", None)
        return {"type": "mcp_server", "name": server["name"], "transport": server.get("transport", "streamable_http"), "display": public_server(server).get("display"), "protocol_version": str(getattr(client, "protocol_version", "")), "server_info": {"name": getattr(info, "name", None) if info else None, "version": getattr(info, "version", None) if info else None}, "tools": tools, "resources": resources, "resource_templates": templates, "prompts": prompts, "execution_enabled": True}
