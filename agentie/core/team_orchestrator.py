...
def create_team_job(task:str,agents:list[dict[str,Any]],requested_by:str="user")->dict[str,Any]:
    ...
    jid="team_"+uuid.uuid4().hex[:10];now=_now();handoffs=[{"id":"ho_"+uuid.uuid4().hex[:8],"from":requested_by,"to_agent_id":a["id"],"to_agent_name":a["name"],"task":task.strip(),"context":{"task":task.strip()},"status":"queued","result":None,"error":None,"attempts":0,"progress_summary":None,"status_checked_at":None} for a in agents];job={"id":jid,"task":task.strip(),"status":"queued","requested_by":requested_by,"agent_ids":[a["id"] for a in agents],"agent_names":[a["name"] for a in agents],"handoffs":handoffs,"created_at":now,"updated_at":now,"final_output":None}
...
async def _worker(job_id:str,handoff:dict[str,Any])->tuple[str,str|None,str|None]:
    ...
    def start(job):
        for h in job["handoffs"]:
            if h["id"]==handoff["id"]:h["status"]="working";h["started_at"]=_now();h["attempts"]=int(h.get("attempts",0))+1;h["error"]=None;h["progress_summary"]=f"Working on {h.get('task') or job.get('task')}.";h["status_checked_at"]=_now()
...
async def _ask_worker_status(job:dict[str,Any],handoff:dict[str,Any])->tuple[str,str]:
    """Ask an active worker for a tiny status update without touching its work session."""
    ...
    except Exception:return handoff["id"],fallback
...
def request_team_status(job_id:str)->dict[str,Any]:
    ...
    updated=_mutate(job_id,apply) or job;card=team_job_card(updated);remember_global_result("",card);return updated
...
def team_job_card(job:dict[str,Any])->dict[str,Any]:
    return {"type":"team_job","id":job["id"],"task":job["task"],"status":job["status"],"agents":job.get("agent_names",[]),"handoffs":[{"id":h["id"],"agent":h["to_agent_name"],"status":h["status"],"error":h.get("error"),"attempts":h.get("attempts",0),"summary":h.get("progress_summary"),"status_checked_at":h.get("status_checked_at")} for h in job.get("handoffs",[])],"final_output":job.get("final_output"),"created_at":job.get("created_at"),"started_at":job.get("started_at"),"finished_at":job.get("finished_at"),"updated_at":job.get("updated_at"),"status_checked_at":job.get("status_checked_at")}
...
def retry_team_worker(job_id:str,agent_name:str)->dict[str,Any]:
    ...
    hid=handoff["id"]
    _mutate(job_id,lambda j:[h.update(status="queued",error=None,progress_summary=None,status_checked_at=None) for h in j["handoffs"] if h["id"]==hid]);start_team_job(job_id,{hid});return get_team_job(job_id) or job
...
def route_team_command(message:str)->dict[str,Any]|None:
    ...
    if _looks_like_status_request(lower):
        id_match=re.search(r"\b(team_[a-z0-9]+)\b",lower);job=get_team_job(id_match.group(1)) if id_match else _latest_job_for_status()
        if not job:return None
        try:job=request_team_status(job["id"])
        except ValueError:return {"message":"Team job not found.","card":None}
        names=", ".join(job.get("agent_names",[]));return {"message":f"Here’s a quick status update from {names or 'the team'}.","card":team_job_card(job)}
