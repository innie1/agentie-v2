...
def _missing_agent_choice(name:str,task:str,requested_agents:list[str]|None=None)->dict[str,Any]:
    similar=_similar_agents(name);options=[{"action":"create_agent","label":f"Create {name}","agent_name":name}]
...
def create_team_job(task:str,agents:list[dict[str,Any]],requested_by:str="user")->dict[str,Any]:
    ...
    handoffs=[{"id":"ho_"+uuid.uuid4().hex[:8],"from":requested_by,"to_agent_id":a["id"],"to_agent_name":a["name"],"task":task.strip(),"context":{"task":task.strip()},"status":"queued","result":None,"error":None,"attempts":0} for a in agents]
...
async def _worker(job_id:str,handoff:dict[str,Any])->tuple[str,str|None,str|None]:
    ...
    prompt=(f"You are {agent['name']}, the {agent['role']} agent. "
            ...
            "Do not assume access to the sender's private chat or memory. Prefer your available local/Python/tools before provider-dependent work when they can complete the task.\n\nTASK:\n"+str(handoff["task"]))
...
def _finish_job(job_id:str,results:list[tuple[str,str|None,str|None]],only_ids:set[str]|None=None)->None:
    ...
    def finish(j):
        ...
        completed=[h for h in j["handoffs"] if h.get("status")=="completed"]
        failed=[h for h in j["handoffs"] if h.get("status")=="failed"]
        active=[h for h in j["handoffs"] if h.get("status") in {"queued","working"}]
...
def retry_team_worker(job_id:str,agent_name:str)->dict[str,Any]:
    ...
    hid=handoff["id"]
    _mutate(job_id,lambda j:[h.update(status="queued",error=None) for h in j["handoffs"] if h["id"]==hid])
    start_team_job(job_id,{hid});return get_team_job(job_id) or job

def team_job_card(job:dict[str,Any])->dict[str,Any]:
    return {"type":"team_job","id":job["id"],"task":job["task"],"status":job["status"],"agents":job.get("agent_names",[]),"handoffs":[{"id":h["id"],"agent":h["to_agent_name"],"status":h["status"],"error":h.get("error"),"attempts":h.get("attempts",0)} for h in job.get("handoffs",[])],"final_output":job.get("final_output")}
...
def route_team_command(message:str)->dict[str,Any]|None:
    ...
    if together:
        ...
        job=create_team_job(task,agents);start_team_job(job["id"]);return {"message":f"Started team job {job['id']} with {', '.join(job['agent_names'])} working simultaneously.","card":team_job_card(job)}
    ...
    if delegate:
        ...
        if missing:return _missing_agent_choice(missing,task,[x.strip() for x in re.split(r"\s*,\s*|\s+and\s+",raw_names,flags=re.I)])
        job=create_team_job(task,agents);start_team_job(job["id"]);return {"message":f"Delegated the task to {', '.join(job['agent_names'])} as team job {job['id']}.","card":team_job_card(job)}
    return None
