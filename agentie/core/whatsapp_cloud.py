...
def _graph_post(payload: dict[str, Any]) -> dict[str, Any]:
    cfg = _credentials();token = str(cfg.get("WHATSAPP_ACCESS_TOKEN") or "").strip()
    if not token:
...
def _record(item: dict[str, Any]) -> dict[str, Any]:
    items = _load_history();message_id = str(item.get("id") or "")
    if message_id:
        existing = next((x for x in items if str(x.get("id") or "") == message_id), None)
        if existing:
            existing.update({k: v for k, v in item.items() if v is not None});_save_history(items);return existing
    items.append(item);_save_history(items);return item


def list_messages(limit: int = 20, phone: str | None = None, direction: str | None = None, needs_human: bool | None = None) -> list[dict[str, Any]]:
    items = _load_history()
    if phone:
        needle = normalize_phone(phone);items = [x for x in items if str(x.get("from") or x.get("to") or "") == needle]
    if direction:
        wanted = str(direction).strip().lower();items = [x for x in items if str(x.get("direction") or "").lower() == wanted]
...
def _agent_signature(agent: dict[str, Any] | None) -> str:
    if not agent:return ""
    name = str(agent.get("name") or "Agent").strip();role = str(agent.get("role") or "Agent").strip();company = str(agent.get("company_identity") or "").strip()
    role_line = role if re.search(r"\bAI\b", role, re.I) else f"AI {role} Agent"
    signature = f"— {name} · {role_line}"
    return f"{signature}\n{company}" if company else signature


def sign_agent_message(text: str, agent: dict[str, Any] | None) -> str:
    body = str(text or "").strip();signature = _agent_signature(agent)
    if not signature:return body
    name = str((agent or {}).get("name") or "").strip()
    if name and name.casefold() in body[-220:].casefold() and "ai" in body[-220:].casefold():return body
    return f"{body}\n\n{signature}".strip()


def _agent_metadata(agent: dict[str, Any] | None) -> dict[str, Any]:
    return {
        "agent_id": (agent or {}).get("id"),
        "agent_name": (agent or {}).get("name"),
        "agent_role": (agent or {}).get("role"),
        "company_identity": (agent or {}).get("company_identity"),
    }


def send_text_message(to: str, text: str, *, agent: dict[str, Any] | None = None) -> dict[str, Any]:
    recipient = normalize_phone(to);body = sign_agent_message(text, agent)
    if not body:raise ValueError("WhatsApp message text is required.")
    if len(body) > 4096:raise ValueError("WhatsApp text messages must be 4096 characters or fewer.")
    result = _graph_post({"messaging_product":"whatsapp","recipient_type":"individual","to":recipient,"type":"text","text":{"preview_url":False,"body":body}})
    ...
    item = _record({
        ...
        "direction":"outgoing","to":recipient,"from":None,"type":"text","body":body,"status":"accepted","at":_now(),"needs_human":False,
        **_agent_metadata(agent),
    })
    return {"sent":True,"message":item,"provider":result}


def send_template_message(to: str, template_name: str, language_code: str = "en_US", components: list[dict[str, Any]] | None = None, *, agent: dict[str, Any] | None = None) -> dict[str, Any]:
    recipient = normalize_phone(to);name = str(template_name or "").strip();language = str(language_code or "en_US").strip()
    if not re.fullmatch(r"[a-z0-9_]{1,512}", name, re.I):raise ValueError("Provide a valid approved WhatsApp template name.")
    template: dict[str, Any] = {"name":name,"language":{"code":language}}
    if components:template["components"] = components
    result = _graph_post({"messaging_product":"whatsapp","to":recipient,"type":"template","template":template})
    message_id = str(((result.get("messages") or [{}])[0] or {}).get("id") or "")
    item = _record({
        "id":message_id or f"local-{hashlib.sha256((recipient + name + _now()).encode()).hexdigest()[:16]}",
        "direction":"outgoing","to":recipient,"type":"template","template_name":name,"body":f"Template: {name}","status":"accepted","at":_now(),"needs_human":False,
        **_agent_metadata(agent),
    })
    return {"sent":True,"message":item,"provider":result}
...
def mark_message_read(message_id: str) -> dict[str, Any]:
    key = str(message_id or "").strip()
    if not key:raise ValueError("WhatsApp message ID is required.")
    result = _graph_post({"messaging_product":"whatsapp","status":"read","message_id":key})
    item = get_message(key)
    if item:item["read"] = True;_record(item)
...
def _save_settings(value: dict[str, Any]) -> None:_save_json(SETTINGS_FILE, value)
...
def set_support_agent(agent_id_or_name: str | None) -> dict[str, Any]:
    ...
    if not agent_id_or_name:
        settings.pop("support_agent_id", None);_save_settings(settings);return settings_snapshot()
    agent = _find_agent(agent_id_or_name)
    if not agent:raise ValueError("Agent was not found.")
    settings["support_agent_id"] = agent["id"];settings["support_mode"] = True;_save_settings(settings);return settings_snapshot()
...
def assign_contact(phone: str, agent_id_or_name: str | None) -> dict[str, Any]:
    number = normalize_phone(phone);settings = _load_settings();contacts = settings.setdefault("contacts", {})
    if not agent_id_or_name:contacts.pop(number, None)
    else:
        agent = _find_agent(agent_id_or_name)
        if not agent:raise ValueError("Agent was not found.")
        contacts[number] = agent["id"]
    _save_settings(settings);return settings_snapshot()
...
def settings_snapshot() -> dict[str, Any]:
    ...
    contacts=[]
    for phone,agent_id in (settings.get("contacts") or {}).items():contacts.append({"phone":phone,"agent":_find_agent(str(agent_id))})
    return {"support_mode":bool(settings.get("support_mode", True)),"support_agent":support,"contacts":contacts}
...
def _default_route_agent() -> dict[str, Any] | None:
    ...
    for pattern in (r"chief of staff", r"customer support|customer service|support", r"manager", r"ceo"):
        for agent in agents:
            if re.search(pattern, f"{agent.get('name','')} {agent.get('role','')}", re.I):return agent
    return agents[0] if agents else None


def route_incoming_agent(phone: str, body: str = "") -> dict[str, Any] | None:
    settings = _load_settings();number = normalize_phone(phone);assigned = (settings.get("contacts") or {}).get(number)
    if assigned:
        agent = _find_agent(str(assigned))
        if agent:return agent
    if settings.get("support_mode", True) is False:return None
...
def _message_body(message: dict[str, Any]) -> str:
    ...
    if kind == "interactive":
        interactive = message.get("interactive") or {};reply = interactive.get("button_reply") or interactive.get("list_reply") or {}
        return str(reply.get("title") or reply.get("id") or "")
    media = message.get(kind) if isinstance(message.get(kind), dict) else {};caption = str(media.get("caption") or "")
...
def _queue_event(item: dict[str, Any]) -> None:
    ...
    events.append({"created_at":_now(),"delivered":False,"message":item});_save_json(EVENTS_FILE, events[-300:])
...
def ingest_webhook(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for entry in payload.get("entry") or []:
        for change in entry.get("changes") or []:
            if str(change.get("field") or "") != "messages":continue
            ...
            for status in value.get("statuses") or []:
                ...
                item = get_message(message_id) or {"id":message_id,"direction":"outgoing"};item["status"] = str(status.get("status") or item.get("status") or "");item["status_at"] = _iso_from_unix(status.get("timestamp"))
                if status.get("errors"):item["status_errors"] = status.get("errors")
                _record(item);statuses += 1
            for message in value.get("messages") or []:
                message_id = str(message.get("id") or "")
                if message_id and get_message(message_id):duplicates += 1;continue
                sender = normalize_phone(str(message.get("from") or ""));body = _message_body(message);routed = route_incoming_agent(sender, body);reason = escalation_reason(body)
                item = _record({
                    "id":message_id or f"webhook-{hashlib.sha256(json.dumps(message,sort_keys=True).encode()).hexdigest()[:16]}",
                    "direction":"incoming","from":sender,"to":str(metadata.get("display_phone_number") or ""),"phone_number_id":str(metadata.get("phone_number_id") or ""),
                    "profile_name":contacts.get(sender) or "","type":str(message.get("type") or "unknown"),"body":body,"at":_iso_from_unix(message.get("timestamp")),
                    "routed_agent_id":(routed or {}).get("id"),"routed_agent_name":(routed or {}).get("name"),"routed_agent_role":(routed or {}).get("role"),
                    "needs_human":bool(reason),"escalation_reason":reason,"read":False,
                })
...
def verify_webhook_challenge(mode: str | None, verify_token: str | None, challenge: str | None) -> str | None:
    expected = str(_credentials().get("WHATSAPP_VERIFY_TOKEN") or "")
    if mode == "subscribe" and expected and hmac.compare_digest(str(verify_token or ""), expected):return str(challenge or "")
    return None


def verify_webhook_signature(raw_body: bytes, signature_header: str | None) -> bool:
    secret = str(_credentials().get("WHATSAPP_APP_SECRET") or "");header = str(signature_header or "")
...
def poll_events(limit: int = 20) -> list[dict[str, Any]]:
    ...
    for event in pending:event["delivered"] = True
    _save_json(EVENTS_FILE, events[-300:]);output=[]
    for event in pending:
        item = event.get("message") or {};who = item.get("profile_name") or item.get("from") or "Unknown contact";route = item.get("routed_agent_name") or "unassigned"
        lines=[f"From: {who}",f"Routed to: {route}",str(item.get("body") or "")]
        if item.get("needs_human"):lines.append(f"Needs human: {item.get('escalation_reason') or 'yes'}")
        output.append({"message":f"New WhatsApp message from {who}.","card":{"type":"note","title":"WhatsApp · Incoming","content":"\n".join(lines)}})
    return output
