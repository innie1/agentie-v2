...
async def run_agent(message:str,agent_type:str="general",session_id:str|None=None)->str:
    """Run one turn. Persistent agents use their configured job/capabilities, not a preset profession."""
    ...
    provider_info=get_provider_info();model_name=provider_info["model"];cooldown=provider_cooldown(provider_info)
    if cooldown:
        remaining=max(1,int(float(cooldown.get("until") or 0)-time.time()));friendly=str(cooldown.get("message") or "The AI model is temporarily at its usage limit. Please try again shortly.");record_event("provider_cooldown",provider_info.get("provider") or "provider",metadata={"model":model_name,"remaining_seconds":remaining,"provider_calls":0})
        if session_id:set_context(session_id,"last_provider_failure",{"user_message":original_message,"error":friendly,"model":model_name,"trace_id":trace_id,"cooldown":True,"remaining_seconds":remaining})
    ...
    if session_id:add_message(session_id,"user",original_message,{"agent_type":"persistent" if persistent_agent else agent_type,"runtime_role":role_info.get("name")})
    record_event("agent",str(role_info.get("name") or agent_type),metadata={"base":"persistent" if persistent_agent else role_info.get("base"),"session_id":session_id});record_event("provider",provider_info["provider"],metadata={"model":model_name});started=time.perf_counter()
    try:
        ...
        if mcp_url and (persistent_agent or str(role_info.get("base")) in {"general","manager"}):
            ...
            async with MCPServerStreamableHttp(name="Agentie MCP",params={"url":mcp_url,"headers":headers},cache_tools_list=True) as server:
                assistant=build_assistant(agent_type,mcp_servers=[server],role_info=role_info,persistent_instructions=persistent_instructions,persistent_agent=persistent_agent);result=await Runner.run(assistant,effective_message)
        else:
            assistant=build_assistant(agent_type,role_info=role_info,persistent_instructions=persistent_instructions,persistent_agent=persistent_agent);result=await Runner.run(assistant,effective_message)
        latency_ms=(time.perf_counter()-started)*1000;record_model_result(result,model_name,latency_ms,trace_id);output=str(result.final_output)
        if session_id:add_message(session_id,"assistant",output,{"agent_type":"persistent" if persistent_agent else agent_type,"runtime_role":role_info.get("name")});set_context(session_id,"last_provider_failure",None)
        if own_trace:finish_trace(trace_id,"completed")
        return output
    except Exception as exc:
        latency_ms=(time.perf_counter()-started)*1000;record_model_error(model_name,exc,latency_ms,trace_id);friendly=_friendly_provider_error(exc)
        if friendly and "usage limit" in friendly.casefold():cooldown=_start_provider_cooldown(provider_info,friendly);record_event("provider_cooldown_started",provider_info.get("provider") or "provider",metadata={"model":model_name,"seconds":cooldown.get("seconds"),"provider_calls":0})
        if session_id:set_context(session_id,"last_provider_failure",{"user_message":original_message,"error":friendly or str(exc),"model":model_name,"trace_id":trace_id})
