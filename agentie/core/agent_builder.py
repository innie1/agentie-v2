...
def draft_agent_spec(description: str, *, name: str = "", job: str = "") -> dict[str, Any]:
    ...
    working_style="Proactive, reliable, clear about uncertainty, and willing to recommend a better approach when evidence supports it";instructions=(f"Job ownership: {job_text}.\nWork from the user's configured goal, responsibilities, knowledge, skills, plugins and approval boundaries. Do not assume a predefined profession or department beyond what the user configured. Use the least costly real capability that can complete the work, and never claim an action succeeded unless it actually did.");plugins=recommend_plugins(description);manager=recommend_manager(description);routines=routine_suggestions(description);delegate=bool(_COORDINATE_RE.search(description))
