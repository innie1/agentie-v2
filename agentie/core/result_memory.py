...
def remember_result(session_id: str, message: str, card: dict[str, Any] | None) -> None:
    ...
    data[session_id] = items[-MAX_RESULTS_PER_SESSION:]
    _save(data)
...
def _last30days_markdown(item: dict[str, Any]) -> str:
    ...
    topic = str(card.get("topic") or "Recent research").strip()
    answer = str(card.get("answer") or item.get("message") or "").strip()
    counts = card.get("source_counts") or {}
    sources = card.get("sources") or []
    lines = [f"# Last30Days Research: {topic}"]
    if counts:
    ...
    if sources:
        ...
        for source in sources[:20]:
            ...
            lane = str(source.get("source") or "web").strip()
            prefix = f"[{sid}] " if sid else ""
            lines.append(f"- {prefix}{title} — {lane}" + (f" — {url}" if url else ""))
    return "\n".join(lines).strip()


def _team_markdown(item: dict[str, Any]) -> str:
    card = item.get("card") or {}
    lines = [f"# Team Job: {card.get('task') or 'Collaborative work'}", "", f"**Status:** {card.get('status') or 'unknown'}"]
    agents = card.get("agents") or []
    if agents:
        lines.append(f"**Agents:** {', '.join(map(str, agents))}")
    final_output = str(card.get("final_output") or "").strip()
    if final_output:
        lines.extend(["", "## Result", final_output])
    else:
        lines.extend(["", "The team job has not produced a final result yet."])
    return "\n".join(lines)
...
def result_content(item: dict[str, Any] | None) -> str | None:
    ...
    kind = item.get("type")
    if kind == "last30days":
        return _last30days_markdown(item)
    if kind == "team_job":
        return _team_markdown(item)
    card = item.get("card") or {}
