...
from agentie.core.plugin_credentials import public_setup_state, setup_response
...
def _agent_arguments(agent: dict[str, Any] | None) -> dict[str, str]:
    if not agent:return {}
    return {
        "agent_id": str(agent.get("id") or ""),
        "agent_name": str(agent.get("name") or ""),
        "agent_role": str(agent.get("role") or ""),
        "company_identity": str(agent.get("company_identity") or ""),
    }
...
def _settings_card() -> dict[str, Any]:
    ...
    lines = [f"Support mode: {'on' if settings.get('support_mode') else 'off'}",f"Support agent: {support.get('name') or 'automatic'}"]
    contacts = settings.get("contacts") or []
    if contacts:
        lines.append("Contact routing:")
        for item in contacts[:30]:
            agent = item.get("agent") or {};lines.append(f"- {item.get('phone')} → {agent.get('name') or 'unassigned'}")
...
def _history_card(*, needs_human: bool | None = None, limit: int = 20) -> dict[str, Any]:
    items = list_messages(limit=limit, needs_human=needs_human);lines=[]
    for item in items:
        direction = "←" if item.get("direction") == "incoming" else "→";other = item.get("from") if item.get("direction") == "incoming" else item.get("to");who = item.get("profile_name") or other or "unknown"
        body = str(item.get("body") or "").replace("\n", " ")[:180];route = item.get("routed_agent_name") or item.get("agent_name") or "";human = f" · HUMAN: {item.get('escalation_reason') or 'yes'}" if item.get("needs_human") else ""
        lines.append(f"{direction} {who} · {body}{f' · {route}' if route else ''}{human}")
    title = "WhatsApp · Needs human" if needs_human else "WhatsApp · Recent messages";message = "Here are WhatsApp conversations that need human attention." if needs_human else "Here are the recent WhatsApp conversations."
...
def _config_command(text: str) -> dict[str, Any] | None:
    ...
    if low in {"show whatsapp settings","whatsapp settings","show my whatsapp settings"}:return _settings_card()
    if low in {"show whatsapp history","whatsapp history"}:return _history_card()
    if re.search(r"\b(?:show|list)\b.*\bwhatsapp\b.*\b(?:need|needs|needing)\s+human\b", low):return _history_card(needs_human=True)
    match = re.match(r"^(?:set|assign)\s+(?:the\s+)?whatsapp\s+support\s+agent\s+(?:to|as)\s+(.+)$", compact, re.I)
    if match:
        try:state = set_support_agent(match.group(1).strip())
        except ValueError as exc:return {"message":str(exc),"card":None}
        support = state.get("support_agent") or {};return {"message":f"WhatsApp support messages will route to {support.get('name') or 'the selected agent'}.","card":{"type":"note","title":"WhatsApp · Support agent","content":f"Support agent: {support.get('name')} ({support.get('role')})"}}
    ...
    if re.match(r"^(?:disable|turn off)\s+whatsapp\s+support\s+mode$", compact, re.I):
        set_support_mode(False);return {"message":"WhatsApp support mode is disabled. Explicit contact assignments still route, but other incoming messages stay unassigned.","card":{"type":"note","title":"WhatsApp · Support mode","content":"Disabled · contacts-only routing"}}
    match = re.match(r"^(?:assign|route)\s+whatsapp\s+contact\s+(.+?)\s+(?:to|through)\s+(.+)$", compact, re.I)
    if match:
        try:state = assign_contact(match.group(1).strip(), match.group(2).strip())
        except ValueError as exc:return {"message":str(exc),"card":None}
...
def _tool_names(info: dict[str, Any]) -> set[str]:return {str(item.get("name") or "") for item in info.get("tools") or [] if item.get("name")}
...
def _setup_with_retry(text: str, error: str) -> dict[str, Any]:
    response = setup_response("whatsapp", error)
    if isinstance(response.get("card"), dict):response["card"]["retry_command"] = text
    return response


def _approval(tool: str, arguments: dict[str, Any], text: str) -> dict[str, Any]:
    canonical = f"Call MCP whatsapp tool {tool} with {json.dumps(arguments,ensure_ascii=False)}";approval = _approval_response("whatsapp",tool,arguments,canonical,natural=True)
...
async def route_whatsapp(message: str, session_id: str | None = None) -> dict[str, Any] | None:
    ...
    if not text or "whatsapp" not in text.lower():return None
    configured = _config_command(text)
    if configured is not None:return configured
    if not _intent(text):return None
    if not _server_registered():return {"message":"WhatsApp Cloud is not registered yet. Add it from Plugins, configure the Meta credentials, then try again.","card":None}
    setup = public_setup_state("whatsapp")
    if setup.get("requires_credentials") and not setup.get("configured"):
        return _setup_with_retry(text, "WhatsApp Cloud still needs the required Meta credentials.")
    try:info = await inspect_server("whatsapp")
    except Exception as exc:return _setup_with_retry(text, _error_text(exc))

    low = text.lower();agent = _agent_from_session(session_id);agent_args = _agent_arguments(agent)
    if low in {"check whatsapp","check my whatsapp","show whatsapp messages","list whatsapp messages","show my whatsapp messages"} or re.search(r"\b(?:show|list|check|read)\b.*\bwhatsapp\b.*\b(?:messages?|history|inbox|conversations?)\b", low):return _history_card()
    if re.search(r"\b(?:show|list|check)\b.*\b(?:need|needs|needing)\s+human\b", low):return _history_card(needs_human=True)

    if re.search(r"\bmark\b.*\bwhatsapp\b.*\bread\b", low):
        message_id = _message_id(text)
        if not message_id:return {"message":"Tell me which WhatsApp message ID to mark as read.","card":None}
        tool = _tool(info, "mark_whatsapp_read")
        if not tool:return {"message":"The WhatsApp MCP does not expose a mark-read tool.","card":None}
        arguments={"message_id":message_id};approval=_approval(tool,arguments,text)
        if approval.get("approved"):
            try:return await execute_tool("whatsapp",tool,arguments)
            except Exception as exc:return {"message":f"The approved mark-read action could not complete: {_error_text(exc)}","card":None}
        return approval
    ...
    if template_match:
        ...
        arguments = {"to":recipient,"template_name":template_match.group(1),"language_code":template_match.group(3) or "en_US","components_json":"[]",**agent_args};approval=_approval(tool,arguments,text)
        if approval.get("approved"):
            try:return await execute_tool("whatsapp",tool,arguments)
    ...
    if reply_match:
        message_id = reply_match.group(1);source = get_message(message_id) if message_id else _latest_incoming()
        if not source or source.get("direction") != "incoming":return {"message":"I couldn't find that incoming WhatsApp message to reply to.","card":None}
        recipient = str(source.get("from") or "");body = sign_agent_message(reply_match.group(2).strip(" .\"'`"), agent);tool = _tool(info, "send_whatsapp_text")
        if not tool:return {"message":"The WhatsApp MCP does not expose text sending.","card":None}
        arguments = {"to":recipient,"text":body,**agent_args};approval=_approval(tool,arguments,text)
        if approval.get("approved"):
            try:return await execute_tool("whatsapp",tool,arguments)
    ...
    if re.search(r"\b(?:send\s+)?whatsapp\b", low):
        ...
        if not tool:return {"message":"The WhatsApp MCP does not expose text sending.","card":None}
        arguments = {"to":recipient,"text":sign_agent_message(body,agent),**agent_args};approval=_approval(tool,arguments,text)
        if approval.get("approved"):
            try:return await execute_tool("whatsapp",tool,arguments)
