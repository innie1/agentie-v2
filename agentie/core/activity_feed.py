...
def activity_items(*,agent_id:str|None=None,limit:int=60)->list[dict[str,Any]]:
    ...
    # Unique authoritative IDs prevent a trace/tool event being shown twice merely
    # because another store references it.
    unique={str(x["id"]):x for x in rows};items=sorted(unique.values(),key=_sort_key,reverse=True)
