...
def init_db() -> None:
    with _connect() as conn:
        ...
        """)
        try:
            conn.execute("CREATE VIRTUAL TABLE IF NOT EXISTS chunks_fts USING fts5(chunk_id UNINDEXED, collection_id UNINDEXED, filename UNINDEXED, text, tokenize='porter unicode61')")
...
def create_collection(name: str, description: str = "") -> tuple[dict[str, Any], bool]:
    ...
    with _connect() as conn:
        ...
        cid = uuid.uuid4().hex[:10]; now = _now()
        conn.execute("INSERT INTO collections(id,name,description,created_at,updated_at) VALUES(?,?,?,?,?)", (cid, clean[:120], description[:1000], now, now))
...
def list_collections() -> list[dict[str, Any]]:
    init_db()
    with _connect() as conn:
        rows = conn.execute("SELECT c.*, COUNT(d.id) document_count FROM collections c LEFT JOIN documents d ON d.collection_id=c.id GROUP BY c.id ORDER BY c.updated_at DESC").fetchall()
...
def index_file(collection: str, filename: str) -> dict[str, Any]:
    ...
    payload=extract_text(path); text=str(payload.get("text") or "")
    pieces=_chunks(text)
    ...
    with _connect() as conn:
        ...
        if old:
            old_id=str(old["id"]); conn.execute("DELETE FROM chunks WHERE document_id=?",(old_id,))
            try: conn.execute("DELETE FROM chunks_fts WHERE chunk_id IN (SELECT id FROM chunks WHERE document_id=?)",(old_id,))
...
def _query_terms(query: str) -> list[str]:
    return [w for w in re.findall(r"[A-Za-z0-9][A-Za-z0-9_-]{1,}", query.lower()) if w not in {"the","a","an","of","to","in","for","and","or","is","are","this","that","what","how","why"}][:16]
...
def search_collection(collection: str, query: str, limit: int = 6) -> dict[str, Any]:
    ...
    limit=max(1,min(int(limit),12)); terms=_query_terms(query)
    rows=[]
    with _connect() as conn:
        if terms:
            fts_query=" OR ".join(f'"{t}"' for t in terms)
            try:
                rows=conn.execute("SELECT chunk_id,filename,text,bm25(chunks_fts) rank FROM chunks_fts WHERE collection_id=? AND chunks_fts MATCH ? ORDER BY rank LIMIT ?",(col["id"],fts_query,limit*3)).fetchall()
            except sqlite3.OperationalError: rows=[]
        if not rows:
            rows=conn.execute("SELECT id chunk_id,filename,text,0 rank FROM chunks WHERE collection_id=? ORDER BY position LIMIT ?",(col["id"],limit*3)).fetchall()
    qset=set(terms)
    scored=[]
    for r in rows:
        text=str(r["text"]); words=set(_query_terms(text)); overlap=len(qset & words)/max(1,len(qset)); phrase=1.0 if query.lower() in text.lower() else 0.0
        lexical=max(0.0, min(1.0, overlap + phrase*.5)); scored.append((lexical,dict(r)))
    scored.sort(key=lambda x:(-x[0],float(x[1].get("rank") or 0)))
    hits=[]
    for idx,(score,r) in enumerate(scored[:limit],1):
        hits.append({"id":f"C{idx}","chunk_id":r["chunk_id"],"filename":r["filename"],"score":round(score,3),"text":r["text"][:2200]})
...
def build_rag_context(collection: str, query: str, limit: int = 6) -> str:
    result=search_collection(collection,query,limit)
    blocks=[f"[{h['id']}] {h['filename']}\n{h['text']}" for h in result["hits"]]
    return f"Collection: {result['collection']['name']}\nQuestion: {query}\n\n"+"\n\n---\n\n".join(blocks)
...
def route_collection_command(message: str) -> dict[str, Any] | None:
    ...
    if m:
        item,created=create_collection(m.group(1).strip(' \"“”'))
        return {"message":f"{'Created' if created else 'Reused existing'} collection “{item['name']}”.","card":{"type":"collection","duplicate_prevented":not created,**item}}
