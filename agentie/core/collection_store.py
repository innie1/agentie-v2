...
def index_file(collection:str,filename:str)->dict[str,Any]:
    ...
    with _connect() as c:
        ...
        for pos,piece in enumerate(pieces):
            cid=f"{doc_id}:{pos}";c.execute("INSERT INTO chunks(id,document_id,collection_id,filename,position,text) VALUES(?,?,?,?,?,?)",(cid,doc_id,col["id"],path.name,pos,piece))
...
def search_collection(collection:str,query:str,limit:int=6)->dict[str,Any]:
    ...
    with _connect() as c:
        if terms:
            ...
            try:rows=c.execute("SELECT chunk_id,filename,text,bm25(chunks_fts) rank FROM chunks_fts WHERE collection_id=? AND chunks_fts MATCH ? ORDER BY rank LIMIT ?",(col["id"],fts_query,limit*3)).fetchall()
            except sqlite3.OperationalError:rows=[]
        if not rows:rows=c.execute("SELECT id chunk_id,filename,text,0 rank FROM chunks WHERE collection_id=? ORDER BY position LIMIT ?",(col["id"],limit*3)).fetchall()
    qset=set(terms);scored=[]
    for r in rows:
        body=str(r["text"]);words=set(_query_terms(body));overlap=len(qset&words)/max(1,len(qset));phrase=1.0 if query.lower() in body.lower() else 0.0;score=max(0.0,min(1.0,overlap+phrase*.5));scored.append((score,dict(r)))
