...
def _load_global() -> dict[str, Any]:
    try:
        data = json.loads(GLOBAL_ACCESS_FILE.read_text(encoding="utf-8")) if GLOBAL_ACCESS_FILE.exists() else {}
    except Exception:
        data = {}
    return {
        "skills": sorted({str(x).lower() for x in data.get("skills", [])}),
        "mcp_servers": sorted({str(x).lower() for x in data.get("mcp_servers", [])}),
        "updated_at": data.get("updated_at"),
    }


def _save_global(data: dict[str, Any]) -> None:
    GLOBAL_ACCESS_FILE.parent.mkdir(parents=True, exist_ok=True)
    data["updated_at"] = _now()
    GLOBAL_ACCESS_FILE.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
...
def _mutate_agent(agent_id_or_name: str, mutator) -> dict[str, Any]:
    ...
    mutator(target)
    target["updated_at"] = _now(); data["updated_at"] = target["updated_at"]
    agent_registry._save(data)
    return agent_registry.get_agent(str(target["id"])) or {}


def global_skill_allowed(skill_id: str) -> bool:
    return str(skill_id).lower() in set(_load_global()["skills"])


def global_mcp_allowed(server_name: str) -> bool:
    return str(server_name).lower() in set(_load_global()["mcp_servers"])


def set_global_skill_access(skill_id: str, allowed: bool) -> dict[str, Any]:
    sid = str(skill_id or "").strip().lower()
    if sid not in all_skills(): raise ValueError("Skill was not found.")
    data = _load_global(); items = set(data["skills"])
    (items.add if allowed else items.discard)(sid); data["skills"] = sorted(items); _save_global(data)
    return global_access_snapshot()


def set_global_mcp_access(server_name: str, allowed: bool) -> dict[str, Any]:
    ...
    data = _load_global(); items = set(data["mcp_servers"])
    (items.add if allowed else items.discard)(server); data["mcp_servers"] = sorted(items); _save_global(data)
    return global_access_snapshot()


def _permission_sets(agent: dict[str, Any]) -> tuple[set[str], set[str], set[str], set[str]]:
    permissions = dict(agent.get("permissions") or {})
    allowed_skills = {str(x).lower() for x in agent.get("skills", [])}
    blocked_skills = {str(x).lower() for x in permissions.get("blocked_skills", [])}
    allowed_mcps = {str(x).lower() for x in permissions.get("mcp_servers", [])}
    blocked_mcps = {str(x).lower() for x in permissions.get("blocked_mcp_servers", [])}
    return allowed_skills, blocked_skills, allowed_mcps, blocked_mcps
...
def set_skill_access(agent_id: str, skill_id: str, mode: str) -> dict[str, Any]:
    ...
    def mutate(target):
        skills = {str(x).lower() for x in target.get("skills", [])}; permissions = dict(target.get("permissions") or {}); blocked = {str(x).lower() for x in permissions.get("blocked_skills", [])}
        skills.discard(sid); blocked.discard(sid)
        if mode == "allow": skills.add(sid)
        elif mode == "block": blocked.add(sid)
        target["skills"] = sorted(skills); permissions["blocked_skills"] = sorted(blocked); target["permissions"] = permissions
    return _mutate_agent(agent_id, mutate)


def set_mcp_access(agent_id: str, server_name: str, mode: str) -> dict[str, Any]:
    server = str(server_name or "").strip().lower()
    registered = {str(x.get("name") or "").lower() for x in list_servers()}
    if server not in registered: raise ValueError("MCP server is not registered.")
    mode = str(mode or "inherit").lower()
    ...
    def mutate(target):
        permissions = dict(target.get("permissions") or {}); allowed = {str(x).lower() for x in permissions.get("mcp_servers", [])}; blocked = {str(x).lower() for x in permissions.get("blocked_mcp_servers", [])}
        allowed.discard(server); blocked.discard(server)
        if mode == "allow": allowed.add(server)
        elif mode == "block": blocked.add(server)
        permissions["mcp_servers"] = sorted(allowed); permissions["blocked_mcp_servers"] = sorted(blocked); target["permissions"] = permissions
    return _mutate_agent(agent_id, mutate)


def global_access_snapshot() -> dict[str, Any]:
    policy = _load_global(); skills=[]
    for sid, skill in sorted(all_skills().items(), key=lambda item: str(item[1].get("name", item[0])).lower()):
        skills.append({"id":sid,"name":skill.get("name",sid),"description":skill.get("description",""),"allowed":sid in policy["skills"],"enabled":bool(skill.get("enabled")),"capabilities":list(skill.get("capabilities") or [])})
    servers=[{"name":str(x.get("name") or ""),"transport":x.get("transport","streamable_http"),"allowed":str(x.get("name") or "").lower() in policy["mcp_servers"]} for x in list_servers()]
    return {"skills":skills,"mcp_servers":servers,"updated_at":policy.get("updated_at")}
...
def access_snapshot(agent_id_or_name: str) -> dict[str, Any]:
    ...
    for sid, skill in sorted(all_skills().items(), key=lambda item: str(item[1].get("name", item[0])).lower()):
        role_default = "*" in skill.get("agents", []) or str(agent.get("base")) in skill.get("agents", [])
        mode = "block" if sid in blocked else "allow" if sid in explicit else "inherit"
        skills.append({"id":sid,"name":skill.get("name",sid),"description":skill.get("description",""),"capabilities":list(skill.get("capabilities") or []),"permissions":list(skill.get("permissions") or []),"enabled":bool(skill.get("enabled")),"inherited":role_default,"global_allowed":global_skill_allowed(sid),"mode":mode,"effective":skill_allowed(agent,sid)})
    servers=[]
    for item in list_servers():
        name=str(item.get("name") or ""); key=name.lower(); mode="block" if key in blocked_mcps else "allow" if key in explicit_mcps else "inherit"
        servers.append({"name":name,"transport":item.get("transport","streamable_http"),"global_allowed":global_mcp_allowed(name),"mode":mode,"allowed":mcp_allowed(agent,name)})
...
def _permission_card(agent: dict[str, Any], kind: str, capability_id: str, command: str) -> dict[str, Any]:
    ...
    return {"message":f"{agent['name']} needs permission to use {capability_id}. Allow it once, allow it for all agents, or deny it.","card":{"type":"agent_capability_approval","approval":approval,"agent_id":agent["id"],"agent_name":agent["name"],"capability_kind":kind,"capability_id":capability_id,"command":command}}
