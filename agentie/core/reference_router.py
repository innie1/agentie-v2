...
def _job_runner_session(session_id:str,job_id:str,specialist:str)->str:return f"{session_id}:job:{job_id}:{specialist}"
...
def _looks_like_background_job(message:str)->bool:
    text=re.sub(r"\s+"," ",message.strip());lower=text.lower();local_hits=len(re.findall(r"\b(timer|alarm|remind|time|weather|calculate|convert|note|system status|stopwatch|routine)\b",lower));complex_hits=len(_JOB_SIGNAL_RE.findall(lower))
...
def remember_active_from_card(session_id:str,card:dict[str,Any]|None)->None:
    ...
    if card_type in {"timer","alarm","reminder","schedule","uploaded_file","note","task","tasks","job_progress","routine","routines","agent_role","agent_roles"}:
...
def _job_command(session_id:str,message:str)->dict[str,Any]|None:
    ...
    if explicit or _looks_like_background_job(text):
        job=create_job(session_id,goal);set_context(session_id,"active_job_id",job["id"]);start_job(job["id"],_job_step_runner);card=job_card(job);set_context(session_id,"active_object",{"type":"job_progress","card":card});return {"message":f"Started job {job['id']} with {job['total_steps']} planned step(s). You can keep chatting while it runs.","card":card,"routed_by":"job"}
    return None

def try_active_reference(session_id:str,message:str)->dict[str,Any]|None:
    _ensure_jobs_resumed();job_result=_job_command(session_id,message)
    ...
    if object_type in {"timer","alarm"}:
        ...
        if duration and reference_words and (change_words or add_words):
            ...
            new_card=dict(card);new_card.update({"type":object_type,"id":timer_id,"status":refreshed.get("status","running"),"duration_seconds":requested,"due_at":refreshed.get("due_at")});set_context(session_id,"active_object",{"type":object_type,"card":new_card});pretty=int(requested) if float(requested).is_integer() else round(requested,1);return {"message":f"{'Timer' if object_type=='timer' else 'Alarm'} updated to {pretty} seconds from now.","card":new_card,"routed_by":"active_reference"}
