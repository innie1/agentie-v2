...
def get_live_state() -> dict[str, Any]:
    try:
        return json.loads(LIVE_STATE_FILE.read_text(encoding="utf-8")) if LIVE_STATE_FILE.exists() else {"active": False, "status": "idle", "url": "", "detail": "", "frame_version": 0, "stop_requested": False}
    except Exception:
        return {"active": False, "status": "idle", "url": "", "detail": "", "frame_version": 0, "stop_requested": False}


def request_browser_stop() -> dict[str, Any]:
    state = get_live_state(); state["stop_requested"] = True; state["updated_at"] = datetime.now().astimezone().isoformat(timespec="milliseconds")
    LIVE_DIR.mkdir(parents=True, exist_ok=True); LIVE_STATE_FILE.write_text(json.dumps(state, indent=2, ensure_ascii=False), encoding="utf-8"); return state
...
def _url(text: str) -> str | None:
    match = re.search(r"https?://[^\s<>\"']+", str(text or ""), re.I); return match.group(0).rstrip(".,;!?)") if match else None
...
def _looks_browser_request(text: str) -> bool:
    low = text.lower(); return bool(_url(text) and re.search(r"\b(?:check|open|visit|inspect|look at|view|screenshot|snapshot|capture|monitor|watch)\b", low))
...
def routine_always_show(text: str) -> bool:
    return bool(re.search(r"\b(?:always show|show every|every screenshot|every check|always notify|notify every)\b", str(text or "").lower()))
...
async def capture_website(url: str, *, track_change: bool = False) -> dict[str, Any]:
    url = _validate_url(url); SNAPSHOT_DIR.mkdir(parents=True, exist_ok=True); filename = _safe_slug(url); path = SNAPSHOT_DIR / filename; _set_live_state(active=True,status="starting",url=url,detail="Starting browser")
    try:
        async with async_playwright() as p:
            if _stop_requested(): raise RuntimeError("Browser task stopped by user.")
            browser = await p.chromium.launch(headless=True); page = await browser.new_page(viewport={"width":1440,"height":1000})
            try:
                _set_live_state(active=True,status="opening",url=url,detail="Opening page"); response = await page.goto(url,wait_until="domcontentloaded",timeout=30000); await _publish_frame(page,status="loaded",url=url,detail="Page loaded")
                if _stop_requested(): raise RuntimeError("Browser task stopped by user.")
                try: _set_live_state(active=True,status="waiting",url=url,detail="Waiting for page activity to settle"); await page.wait_for_load_state("networkidle",timeout=8000)
                except Exception: pass
                await _publish_frame(page,status="ready",url=url,detail="Page ready")
                if _stop_requested(): raise RuntimeError("Browser task stopped by user.")
                title = await page.title(); text = _normalized_text(await page.locator("body").inner_text(timeout=10000)); _set_live_state(active=True,status="capturing",url=url,detail="Capturing full-page screenshot"); await page.screenshot(path=str(path),full_page=True); await _publish_frame(page,status="captured",url=url,detail="Screenshot captured"); status = response.status if response else None
            finally: await browser.close()
    except Exception as exc:
        message = str(exc); _set_live_state(active=False,status="error",url=url,detail="Browser task ended",error=message[:500])
        if "Executable doesn't exist" in message or "playwright install" in message.lower(): raise RuntimeError("Chromium is not installed for Agentie yet. Run: python -m playwright install chromium") from exc
    ...
    excerpt=text[:900]; _set_live_state(active=False,status="done",url=url,detail="Browser task complete")
