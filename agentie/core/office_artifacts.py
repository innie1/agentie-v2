...
def _safe_filename(name: str | None, suffix: str, prefix: str) -> str:
    if not name:
        return f"{prefix}-{datetime.now().strftime('%Y%m%d-%H%M%S')}{suffix}"
    value = Path(name.strip()).name
    if not value.lower().endswith(suffix): value += suffix
    return re.sub(r"[^A-Za-z0-9._ -]+", "-", value)[:160] or f"{prefix}{suffix}"
...
def _resolve_content(session_id: str, message: str) -> str | None:
    ...
    if content is None and (_REFERENCE_RE.search(message) or len(message.split()) <= 14):
        content = latest_assistant_text(session_id, max_chars=120000)
    return content
...
def create_docx(content: str, filename: str | None = None) -> dict[str, Any]:
    if not content.strip(): raise ValueError("Word document content is empty.")
    UPLOADS.mkdir(parents=True, exist_ok=True); path = unique_path(_safe_filename(filename, ".docx", "agentie-document"))
    doc = Document(); title_used = False
    for raw in content.replace("\r\n", "\n").split("\n"):
        line = raw.strip()
        if not line: continue
        if line.startswith("# "):
            doc.add_heading(_plain(line), level=0 if not title_used else 1); title_used = True
    ...
    doc.core_properties.author = "Agentie"; doc.core_properties.title = path.stem.replace("-", " ").title(); doc.save(path)
...
def _tabular_rows(content: str) -> list[list[str]]:
    ...
    try:
        sample=content[:4096]; dialect=csv.Sniffer().sniff(sample, delimiters=",\t;")
    ...
    lines=[_plain(x) for x in content.splitlines() if _plain(x)]
    return [["Content"], *[[x] for x in lines[:10000]]]


def create_xlsx(content: str, filename: str | None = None) -> dict[str, Any]:
    if not content.strip(): raise ValueError("Spreadsheet content is empty.")
    UPLOADS.mkdir(parents=True, exist_ok=True); path=unique_path(_safe_filename(filename,".xlsx","agentie-spreadsheet"))
    rows=_tabular_rows(content); wb=Workbook(); ws=wb.active; ws.title="Agentie"
    ...
    for col in range(1,max((len(r) for r in rows),default=1)+1):
        width=min(50,max(12,max((len(str(ws.cell(r,col).value or "")) for r in range(1,min(ws.max_row,200)+1)),default=12)+2))
        ws.column_dimensions[get_column_letter(col)].width=width
    ws.freeze_panes="A2" if ws.max_row>1 else None; wb.save(path)
    return inspect_file(path)
...
def _sections(content: str) -> list[tuple[str,list[str]]]:
    ...
    if current_title or bullets: sections.append((current_title or "Overview",bullets[:8]))
    if not sections:
        words=[x.strip() for x in re.split(r"(?<=[.!?])\s+",content) if x.strip()]
        sections=[("Overview",words[:6])]
    return [(title,[]),*sections[:18]]


def create_pptx(content: str, filename: str | None = None) -> dict[str, Any]:
    if not content.strip(): raise ValueError("Presentation content is empty.")
    UPLOADS.mkdir(parents=True, exist_ok=True); path=unique_path(_safe_filename(filename,".pptx","agentie-presentation"))
    prs=Presentation(); prs.slide_width=Inches(13.333); prs.slide_height=Inches(7.5); sections=_sections(content)
    title,_=sections[0]; slide=prs.slides.add_slide(prs.slide_layouts[0]); slide.shapes.title.text=title; slide.placeholders[1].text="Generated locally by Agentie"
...
def try_office_request(session_id: str, message: str) -> dict[str, Any] | None:
    ...
    if kind=="docx":card=create_docx(content,_extract_filename(message,".docx"))
    elif kind=="xlsx":card=create_xlsx(content,_extract_filename(message,".xlsx"))
    else:card=create_pptx(content,_extract_filename(message,".pptx"))
