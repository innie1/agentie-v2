...
def _save_state(data):
    STATE.parent.mkdir(parents=True,exist_ok=True);STATE.write_text(json.dumps(data,indent=2,ensure_ascii=False),encoding="utf-8")
def _load_custom()->dict[str,dict[str,Any]]:
    ...
    for path in SKILLS_DIR.glob("*/skill.json"):
        try:
            item=json.loads(path.read_text(encoding="utf-8"));sid=str(item.get("id") or path.parent.name).lower();item["id"]=sid;out[sid]=item
...
def create_skill_manifest(skill_id:str,name:str,description:str,agents:list[str],capabilities:list[str])->dict[str,Any]:
    ...
    item={"id":sid,"name":name[:120],"description":description[:1000],"agents":sorted(set(agents)),"capabilities":sorted(set(capabilities)),"enabled":True,"version":"1.0"};path.write_text(json.dumps(item,indent=2,ensure_ascii=False),encoding="utf-8");return item
def skills_for_agent(agent_type:str)->list[dict[str,Any]]:
    return [s for s in list_skills() if s.get("enabled") and (agent_type in (s.get("agents") or []) or "*" in (s.get("agents") or []))]
