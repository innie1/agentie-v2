...
def resolve_role(agent_name:str)->dict[str,str]:
    data=_load(); assigned=str(data.get("assignments",{}).get(agent_name,"")).strip().lower(); roles={**ROLE_PRESETS,**data.get("custom_roles",{})}
    if assigned and assigned in roles:return {"name":assigned,**roles[assigned]}
    base=agent_name if agent_name in BASE_AGENTS else "general"
    return {"name":base,"base":base,"instruction":f"Act in the {base} role for this task."}
...
def assign_role(agent_name:str,role_name:str)->dict[str,str]:
    ...
    if role_name not in roles:
        # Custom roles inherit general tools unless explicitly associated with a familiar role word.
        base="general"
    ...
    data.setdefault("assignments",{})[agent_name]=role_name;data["updated_at"]=datetime.now().astimezone().isoformat(timespec="seconds");_save(data)
...
def route_role_command(message:str)->dict[str,Any]|None:
    ...
    if not m:
        m=re.search(r"\b(general|research|coding|manager|github)(?:\s+agent)?\s+should\s+(?:act|work|serve)\s+as\s+(?:a|an|the)?\s*([a-z][a-z0-9 -]{1,50})\b",lower)
    if m:
        agent,role=m.group(1),m.group(2).strip(" .");resolved=assign_role(agent,role)
        return {"message":f"{agent.title()} agent is now acting as {resolved['name']}.","card":{"type":"agent_role","agent":agent,**resolved}}
