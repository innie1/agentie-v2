...
_CATEGORY_TERMS = {
    "finance": {"rent", "budget", "cost", "costs", "expense", "expenses", "revenue", "profit", "margin", "cash", "capital", "salary", "salaries", "loan", "tax", "taxes", "price", "pricing", "naira", "₦", "electricity cost", "bill"},
    "marketing": {"marketing", "advert", "advertising", "campaign", "brand", "branding", "promotion", "promote", "social media", "target", "audience", "students", "offices", "awareness", "content", "instagram", "facebook", "x account"},
    ...
    "operations": {"operations", "operation", "washing machine", "machine", "equipment", "electricity", "generator", "delivery", "staff", "supplier", "suppliers", "inventory", "stock", "process", "workflow", "hours", "location", "laundry", "logistics", "procurement", "maintenance"},
...
def _is_manager(agent: dict[str, Any]) -> bool:
    text=f"{agent.get('name','')} {agent.get('role','')} {agent.get('purpose','')} {agent.get('base','')}".casefold();return agent.get("base")=="manager" or bool((agent.get("permissions") or {}).get("delegate")) or any(term in text for term in _MANAGER_TERMS)
def _agent_matches_categories(agent: dict[str, Any], categories: list[str]) -> bool:
    if _is_manager(agent):return True
    text=f"{agent.get('name','')} {agent.get('role','')} {agent.get('purpose','')} {agent.get('base','')}".casefold()
    for category in categories:
        if category!="general" and any(term in text for term in _CATEGORY_AUDIENCES.get(category,set())):return True
    return False
def _routing_agents(categories: list[str]) -> list[dict[str, Any]]:return [a for a in list_agents() if _agent_matches_categories(a,categories)]
...
def update_company_knowledge(key: str, value: str) -> dict[str, Any]:
    ...
    meta=_metadata(row);meta.update({"categories":_categories(clean),"source":"user_edit","approved":True,"shared":True,"routed_by":_chief_of_staff_name() or "Agentie local knowledge router"});set_memory(COMPANY_SCOPE,str(row["key"]),clean,meta);refreshed=_find_company_row(str(row["key"]));return _row_card(refreshed or row)
...
def company_context_for_agent(agent: dict[str, Any], query: str, limit: int = 5) -> str:
    if not agent:return ""
    permissions=agent.get("permissions") or {};shared=permissions.get("shared_company_memory","read")
    if shared in {False,None,"none","block","deny","off"}:return ""
    qterms=_terms(query);scored=[]
    for row in list_memories(COMPANY_SCOPE,200):
        meta=_metadata(row);raw_categories=meta.get("categories") or ["general"];categories=[str(x) for x in raw_categories if str(x)]
...
def _project_brain_dump(project_name: str, body: str) -> dict[str, Any]:
    ...
    for statement in _split_dump(body):
        categories=_categories(statement);audience_terms=set()
        for category in categories:audience_terms.update(_CATEGORY_AUDIENCES.get(category,set()))
        audience_terms.update(_MANAGER_TERMS);append_project_item(project["id"],"knowledge",statement,{"source":"user_brain_dump","shared":True,"audience":",".join(sorted(audience_terms)) or "all","categories":categories});items.append(statement)
...
def route_company_knowledge_command(message: str) -> dict[str, Any] | None:
    ...
    if agent_view:
        ...
        for row in list_memories(COMPANY_SCOPE,200):
            meta=_metadata(row);raw_categories=meta.get("categories") or ["general"];categories=[str(x) for x in raw_categories if str(x)]
            if _agent_matches_categories(agent,categories):items.append(_row_card(row))
    ...
    if delete:
        ...
        if not approval_is_granted(action):
            approval=create_approval(action,f"Permanently remove this company knowledge item: {str(row.get('value') or '')[:240]}",{"kind":"company_knowledge_delete","knowledge_id":key});return {"message":"Removing company knowledge is permanent. Approve the deletion to continue.","card":{"type":"approvals","items":[approval]}}
