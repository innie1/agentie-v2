...
class WSLDesktopRegressionTests(unittest.TestCase):
    def test_urls_use_wsl_host(self):
        novnc, cdp = wsl_desktop._urls("172.21.18.153")
        self.assertTrue(novnc.startswith("http://172.21.18.153:6080/"))
    ...
    def test_start_script_uses_tcp_x_and_wsl_ip_bridge(self):
        ...
        self.assertIn("WSL_IP=", script)
        self.assertIn("$WSL_IP:6080", script)
    ...
    def test_status_checks_wsl_ip(self):
        with patch.object(wsl_desktop, "_windows_wsl", return_value="wsl.exe"), patch.object(wsl_desktop, "_wsl_ip", return_value="172.21.18.153"), patch.object(wsl_desktop, "_port_open", return_value=True) as port_open, patch.object(wsl_desktop, "_http_ready", return_value=True) as http_ready:
            result = wsl_desktop.status()
        port_open.assert_called_once_with("172.21.18.153", 6080)
        http_ready.assert_called_once_with("http://172.21.18.153:9222/json/version")
        self.assertEqual(result["wsl_ip"], "172.21.18.153")
        self.assertTrue(result["running"])

    def test_ready_desktop_does_not_restart(self):
        ready = {"supported": True, "running": True, "novnc_ready": True, "chrome_ready": True, "novnc_url": "http://172.21.18.153:6080/vnc_lite.html", "cdp_url": "http://172.21.18.153:9222", "distro": "Ubuntu", "wsl_ip": "172.21.18.153"}
    ...
    def test_missing_bridge_packages_bootstrap_once(self):
        ...
        not_ready = {"supported": True, "running": False, "novnc_ready": False, "chrome_ready": False, "novnc_url": None, "cdp_url": None, "distro": "Ubuntu", "wsl_ip": "172.21.18.153"}
        ready = {"supported": True, "running": True, "novnc_ready": True, "chrome_ready": True, "novnc_url": "http://172.21.18.153:6080/vnc_lite.html", "cdp_url": "http://172.21.18.153:9222", "distro": "Ubuntu", "wsl_ip": "172.21.18.153"}
    ...
    def test_stop_kills_desktop_services(self):
        stopped = {"supported": True, "running": False, "novnc_ready": False, "chrome_ready": False, "novnc_url": None, "cdp_url": None, "distro": "Ubuntu", "wsl_ip": "172.21.18.153"}
