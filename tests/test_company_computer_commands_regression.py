...
class CompanyComputerCommandsRegressionTests(unittest.TestCase):
    def test_normal_terminal_command_runs_inside_guest_as_agentie(self):
        ...
        with patch.object(commands, "ensure_guest_runtime", return_value={}) as prepare, patch.object(commands.computer, "acquire_agent") as acquire, patch.object(commands.computer, "release_control") as release, patch.object(commands.computer, "guest_exec", return_value=result) as guest, patch.object(commands, "approval_is_granted", return_value=False):
    ...
    def test_approved_system_change_runs_as_root_and_consumes_once(self):
        ...
        with patch.object(commands, "ensure_guest_runtime", return_value={}), patch.object(commands, "approval_is_granted", return_value=True), patch.object(commands, "consume_approval") as consume, patch.object(commands.computer, "acquire_agent"), patch.object(commands.computer, "release_control"), patch.object(commands.computer, "guest_exec", return_value=result) as guest:
            response = commands.run_guest_command("apt-get install -y inkscape", "agent:agt_ops:main")
    ...
    def test_destructive_external_write_and_privilege_escalation_are_approval_gated(self):
        for command in ("rm -rf /home/agentie/old", "git push origin main", "sudo bash -c 'touch /etc/agentie-test'", "pkexec sh -c 'id'"):
            with self.subTest(command=command), patch.object(commands, "approval_is_granted", return_value=False), patch.object(commands, "create_approval", return_value={"id": "approval", "status": "pending"}), patch.object(commands.computer, "guest_exec") as guest, patch.object(commands, "ensure_guest_runtime") as prepare:
    ...
    def test_file_transfer_commands_prepare_guest_and_use_real_qga_file_layer(self):
        ...
        with patch.object(commands, "ensure_guest_runtime", return_value={}) as prepare, patch.object(commands, "upload_workspace_file", return_value=upload) as put:
        ...
        with patch.object(commands, "ensure_guest_runtime", return_value={}) as prepare, patch.object(commands, "download_guest_file", return_value=download) as get:
    ...
    def test_app_launch_occurs_inside_prepared_guest_not_host(self):
        result = {"exited": True, "exitcode": 0}
        status = {"computer_id": "company-default", "state": "IDLE", "persistent": True}
        with patch.object(commands, "ensure_guest_runtime", return_value=status) as prepare, patch.object(commands.computer, "acquire_agent") as acquire, patch.object(commands.computer, "release_control") as release, patch.object(commands.computer, "guest_exec", return_value=result) as guest, patch.object(commands.computer, "status", return_value=status):
