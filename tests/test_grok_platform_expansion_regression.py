...
class GrokPlatformExpansionRegressionTests(unittest.TestCase):
    ...
    def test_taught_skill_keeps_deterministic_browser_replay(self):
        ...
        fake={'id':'wf_123','name':'Publish Update','steps':[]}
        sentinel={'message':'deterministic','card':{'type':'browser_actions'}}
    ...
    def test_builder_recommends_real_team_schedule_and_connections_without_hidden_authority(self):
        coordinator=agent_registry.create_agent('Cora','Operations coordinator',permissions={'delegate':True})['agent']
        agent_registry.create_agent('Mira','Customer research owner',purpose='Research customer complaints and needs')
    ...
    def test_multi_agent_mentions_create_one_real_parallel_team_job(self):
        ...
        with patch.object(agent_threads,'start_team_job') as start:
            row=agent_threads.post_message(thread['id'],'user',None,'User','@Ada @Ben compare today’s customer issues')
    ...
    def test_completed_handoffs_materialize_as_agent_replies_exactly_once(self):
        ...
        with patch.object(agent_threads,'start_team_job'):
            origin=agent_threads.post_message(thread['id'],'user',None,'User','@Ada @Ben review this')
        ...
        team_orchestrator._mutate(jid,finish)
        first=agent_threads.thread_card(agent_threads.get_thread(thread['id']));second=agent_threads.thread_card(agent_threads.get_thread(thread['id']))
    ...
    def test_natural_event_routine_creation_uses_existing_routine_engine(self):
        ...
        result=routine_engine.route_routine_command('Create a routine called Follow Up that when a skill run completes summarize the result',owner_agent_id=agent['id'])
        item=result['card'];self.assertEqual(item['trigger_type'],'event');self.assertEqual(item['event_type'],'skill_run.completed');self.assertEqual(item['owner_agent_id'],agent['id']);self.assertEqual(item['action'],'summarize the result')
    ...
    def test_linked_skill_routine_executes_under_owner_agent_identity(self):
        skill=self._skill();agent=self._agent_with_skill(skill);routine,_=routine_engine.create_routine('Create a routine called Daily Skill that daily at 9 AM run the saved skill',owner_agent_id=agent['id'],skill_id=skill['id'])
        completed={'status':'completed','message':'done','card':{'type':'note'},'run':{'id':'skillrun_test'}}
        with patch.object(workflow_browser_runtime,'_run_skill',new=AsyncMock(return_value=completed)) as run:
            asyncio.run(routine_worker._run_linked_skill(routine))
        run.assert_awaited_once();self.assertEqual(run.await_args.args[0],skill['id']);self.assertTrue(run.await_args.args[1].startswith(agent['session_prefix']+'routine:'))
    ...
    def test_approval_resolution_and_team_completion_publish_internal_events(self):
        approval=approval_tools.create_approval('test:danger','Need a decision',{'agent_id':'agt_test','agent_name':'Ada','kind':'test'})
        approval_tools.resolve_approval(approval['id'],False);self.assertTrue(any(x['type']=='approval.resolved' for x in automation_events.recent_events()))
    ...
    def test_activity_feed_merges_authoritative_existing_stores(self):
        ...
        routine,_=routine_engine.create_routine('Create a routine called Daily Summary that daily at 9 AM summarize activity',owner_agent_id=agent['id']);routine_engine.record_run(routine['id'],None,'completed')
        team_orchestrator.create_team_job('Review customer issues',[agent]);approval_tools.create_approval('mcp:test:send:{}','Allow send',{'kind':'mcp','agent_id':agent['id'],'agent_name':agent['name'],'server':'test','tool':'send'})
    ...
    def test_activity_command_and_platform_ui_expose_new_primitives_cleanly(self):
        ...
        self.assertIs(result['card'],sentinel)
        ui=Path('frontend/platform.js').read_text(encoding='utf-8');browser=Path('agentie/core/workflow_browser_runtime.py').read_text(encoding='utf-8')
