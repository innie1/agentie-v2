...
class ModelRouterRegressionTests(unittest.TestCase):
    ...
    def test_auto_prefers_local_for_simple_work(self):
        route = model_routing.choose_model_route(
    ...
    def test_manual_local_never_allows_cloud_fallback(self):
        route = model_routing.choose_model_route(
            "Commit and push this repository change.",
            mode="local",
            local_available=True,
            cloud_configured=True,
        )
    ...
    def test_auto_best_effort_local_when_complex_but_cloud_is_not_configured(self):
        route = model_routing.choose_model_route(
            "Debug this codebase and implement the fix.",
            mode="auto",
            local_available=True,
    ...
    def test_local_provider_is_openai_compatible_and_configurable(self):
        with patch.dict(os.environ, {
            "AGENTIE_LOCAL_MODEL": "gemma4:test",
            "AGENTIE_LOCAL_BASE_URL": "http://127.0.0.1:9999/v1/",
        }, clear=False):
    ...
    def test_auto_runner_retries_once_on_safe_local_runtime_failure(self):
        route = {
            "mode": "auto", "tier": "local", "reason": "local_default",
            "local_available": True, "cloud_configured": True,
            "task": {"score": 0, "reasons": []}, "allow_cloud_fallback": True,
        }
        ...
        with patch.object(runner, "choose_model_route", return_value=route), \
             ...
             patch.object(runner, "_attempt", new=AsyncMock(side_effect=[RuntimeError("connection refused"), "cloud answer"])), \
        ...
        self.assertEqual(runner._attempt.await_count, 2)
    ...
    def test_local_runner_does_not_retry_cloud(self):
        route = {
            "mode": "local", "tier": "local", "reason": "manual_local",
            "local_available": True, "cloud_configured": True,
            "task": {"score": 0, "reasons": []}, "allow_cloud_fallback": False,
        }
        local = {"provider": "local", "tier": "local", "model": "gemma4", "base_url": "http://127.0.0.1:11434/v1"}
