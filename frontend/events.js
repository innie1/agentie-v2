(() => {
  ...
  function renderWebSearch(card){
    ...
    sources.forEach((source,index)=>{
      const b=document.createElement('button');b.type='button';b.className='web-source-dot';b.title=source.domain||source.title||`Source ${index+1}`;b.setAttribute('aria-label',`View source ${index+1}: ${source.title||source.domain||''}`);
      const fallback=document.createElement('span');fallback.className='source-fallback';fallback.textContent=String(index+1);b.appendChild(fallback);
      const favicon=sourceFavicon(source);
      if(favicon){const img=document.createElement('img');img.alt='';img.loading='lazy';img.referrerPolicy='no-referrer';img.onload=()=>{fallback.style.display='none'};img.onerror=()=>{img.remove();fallback.style.display='inline-flex'};img.src=favicon;b.prepend(img)}
      b.onclick=()=>showSource(source,b);strip.appendChild(b)
    });
