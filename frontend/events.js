(() => {
  ...
  window.renderCard=function(card,message){
    ...
    if(card&&card.type==='job_progress'&&card.id)return renderJob(card);
  ...
  // Due reminders and schedules live in the side rail rather than creating a second
  // chat message. Routine starts get one live job card, then continue updating in place.
