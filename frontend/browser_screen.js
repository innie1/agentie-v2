...
(()=>{
  ...
  let row=null,root=null,starting=null,stopped=false,frameObserver=null;
  ...
  function fitFrame(){if(!root)return;const screen=root.querySelector('.arc-screen'),frame=root.querySelector('.arc-frame');if(!screen||!frame)return;const w=screen.clientWidth,h=screen.clientHeight;if(!w||!h)return;const scale=Math.min(w/1440,h/900);frame.style.transform=`scale(${scale})`;frame.style.left=`${Math.max(0,(w-1440*scale)/2)}px`;frame.style.top=`${Math.max(0,(h-900*scale)/2)}px`}
  function place(){if(!row){row=makeComputer();root=row.querySelector('.agentie-real-computer');wire()}if(!row.isConnected)messages.appendChild(row);requestAnimationFrame(fitFrame);return root}
  ...
  function connect(url){place();const frame=root.querySelector('.arc-frame');frame.onload=()=>{fitFrame();try{frame.focus()}catch(_){}};frame.src=url+(url.includes('?')?'&':'?')+'t='+Date.now();root.querySelector('.arc-overlay').classList.add('hidden');stopped=false;setState('on','KasmVNC Linux desktop · mouse and keyboard control enabled')}
  ...
  function wire(){const screen=root.querySelector('.arc-screen');root.querySelector('.arc-stop').onclick=stopComputer;root.querySelector('.arc-fullscreen').onclick=()=>{if(screen.requestFullscreen)screen.requestFullscreen().catch(()=>{})};if('ResizeObserver'in window){frameObserver=new ResizeObserver(()=>fitFrame());frameObserver.observe(screen)}document.addEventListener('fullscreenchange',()=>setTimeout(fitFrame,50))}
